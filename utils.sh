...
patch_apk() {
	...
	local cmd="java -jar $rv_cli_jar patch $stock_input -p -o $patched_apk -b $rv_patches_jar  $patcher_args \
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		cp -a $MODULE_TEMPLATE_DIR/. "$base_template" && mkdir -p "${base_template}/app"
		local upj="${table,,}-update.json"

		module_config "$base_template" "$app_name" "$pkg_name" "$version" "$arch"
		module_prop \
			"${args[module_prop_name]}" \
			"${app_name} ${args[rv_brand]}" \
			"$version" \
			"${app_name} ${args[rv_brand]} Dynamic Mount. Require Magisk Process Monitor tool v2.3+ for Dynamic Mount." \
		...
		if [ ! -f "$module_output" ] || [ "$REBUILD" = true ]; then
			...
			cp -f "$patched_apk" "${base_template}/app/base.apk"
			if [ "${args[include_stock]}" = true ]; then cp -f "$stock_apk" "${base_template}/app/${pkg_name}.apk"; fi
...
author=j-hc, HuskyDG, changhuapeng
